    ...
    for r in rooms:
        if random.randint(1, 2) == 1:
            ...
            for point in new_corridor.corridor_points:
                for e in rooms:
                    if Algorithim.is_point_inside_room(point, e) and e is not r and e not in new_corridor.connected_rooms:
                        new_corridor.connected_rooms = [r, e]
                        r.connections.append([new_corridor, e])
                        e.connections.append([new_corridor, r])
                        continue
                for t in corridors:
                    if Algorithim.is_point_inside_corridor(point, t) and t is not new_corridor and t not in new_corridor.connected_corridors:
